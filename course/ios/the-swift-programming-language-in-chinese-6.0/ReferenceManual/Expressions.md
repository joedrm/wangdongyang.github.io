...
关于这些运算符的更多信息，请参阅 BasicOperators> 和 AdvancedOperators>.
...
输入输出表达式也可以用于将非指针实参传入到需要指针的上下文中，如 Functions#输入输出参数>.

In-out 表达式也可以用于将非指针实参传入到需要指针的上下文中，如 Expressions#隐式转换为指针类型>.
...
更多关于 `try`、`try?` 和 `try!` 的信息，以及该如何使用的例子，请参阅 ErrorHandling>.
...
关于这些运算符的更多信息，请参阅BasicOperators> 和 AdvancedOperators>.
...
关于使用三元条件运算符的例子，请参阅 BasicOperators#三元运算符>.
...
关于类型转换的更多内容和例子，请参阅 TypeCasting>.
要查看使用类型转换运算符的示例，请参阅 TypeCasting#类型转换运算符>.
...
闭包的参数声明形式跟函数一样，请参阅 Declarations#函数声明>.
...
关于如何将闭包作为参数来传递的内容，请参阅 Expressions#函数调用表达式>.
...
关于闭包表达式的更多信息和例子，请参阅 Closures#闭包表达式>.

关于捕获列表的更多信息和例子，请参阅 AutomaticReferenceCounting#闭包引起的循环强引用>.
...
隐式成员表达式可以跟在后缀运算符或者其他在Expressions#后缀表达式>里介绍的语法后面。这被称为 链式隐式成员表达式。尽管链式后缀表达式大多都是相同类型，但其实只需要整个链式成员表达式可以转换为上下文的类型就行了。更具体的，如果隐式类型是可选的，则可以使用非可选类型的值，如果隐式类型是类类型，则可以使用其子类的值。例如：
...
你使用宏表达式来调用独立宏。要调用附加宏，请使用在 Attributes>.
...
如 Declarations#具有特殊名称的方法>所述，通过声明几种方法中的一种，类、结构体或枚举类型可以为函数调用语法启用语法糖。
...
使用 `dynamicMemberLookup` 属性声明的类型包含可以在运行时查找的成员，具体请参阅 Attributes>.
...
如果点号（`.`）出现在行首，它会被视为显式成员表达式的一部分，而不是 Expressions#隐式成员表达式> 的一部分。例如如下代码所展示的被分为多行的链式方法调用：
...
关于下标的声明，请参阅 Declarations#协议下标声明>.
