...
实例方法有着和函数完全一样的语法，具体描述可以参见 Functions>。
...
函数参数可以同时拥有一个参数名称（在函数体内使用）和一个参数标签（在调用函数时使用），参见Functions#函数参数标签和参数名称>。
