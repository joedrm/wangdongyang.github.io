...
> 关于这些函数语法上的变化详见下方 Functions#函数参数标签和参数名称> 和 Functions#默认参数值>。
...
正如你将会在 Properties#简化-Setter-声明> 里看到的，属性的 getter 也可以使用隐式返回。
...
您可以通过在参数类型前添加 `inout` 关键字来编写输入输出参数。输入输出参数有一个值，该值 **传入** 给函数，被函数修改后，然后被 **传出** 函数，以替换原来的值。有关传入传出参数行为及相关编译器优化的详细讨论，请参阅 Declarations#In-Out-参数>。
